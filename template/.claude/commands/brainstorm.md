---
description: Explore intent and design before implementing, checking against existing project docs
argument-hint: [idea or problem to explore]
---

Help turn this idea into an approved design through dialogue: $ARGUMENTS
...
7. **Persist the approved design.** Once the user approves, write it to
   ...
   **Status:** Approved
   ...
   ## Open questions
   ## Plan
   (added once /plan runs: docs/plans/YYYY-MM-DD-<topic>.md)
   ```

   This is a per-task design record, distinct from `docs/` (which holds
   standing facts about the project as a whole — see `CLAUDE.md`'s "Project
   docs" section). A spec's relevance fades once implemented; it's an archive,
   not something `/update-docs` keeps current. If a decision in it is
...
8. **Self-review before handing off.** Read the written spec file fresh and
   ...
   - **Scope:** is this focused enough for one `/plan` run, or does it
...
9. **User reviews the written file.** Don't treat conversational approval and
   ...
   moving to `/plan`: *"Spec written to `docs/specs/<file>`. Take a look and
...
`docs/specs/`, ready to hand to `/plan`. If an ADR was warranted, it's either
written (with confirmation) or explicitly deferred.

## Principles

Apply the working principles from `CLAUDE.md`: think before coding, simplicity
