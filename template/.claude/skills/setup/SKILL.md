---
description: Interview the user about this project and generate project-specific docs (overview, tech stack, architecture, runbook, constitution, ADR) plus fill CLAUDE.md placeholders. Use once when adopting the harness in a project, or to refresh docs later.
---
...
`/brainstorm`, which is task-scoped and run for every feature.
...
2. **Interview the user**, one question at a time, covering what each
   ...
   - **Does this project follow TDD** (write a failing test before
     ...
     see the `tdd` skill for the red-green-refactor loop."). `/implement`
     checks this rule and invokes the `tdd` skill when present — if it's not
     recorded, `/implement` won't assume TDD.
