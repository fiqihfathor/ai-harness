---
name: implement
description: Use when executing an approved plan step by step, then checking doc impact
---
...
3. Work through the plan's steps **in order, sequentially, by default** —
   ...
   mention the option and dispatch each using your harness's subagent mechanism
   if available; otherwise do it inline sequentially — but don't do this automatically;
   shared state is easy to get wrong, and silent parallel execution makes the
